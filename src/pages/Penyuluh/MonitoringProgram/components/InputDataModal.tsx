...
const InputDataModal: React.FC<Props> = ({ item, penugasanId, onClose, onSaved }) => {
  ...
  const handleSubmit = async () => {
    if (!file) {
      toast.error('Foto monitoring wajib diunggah.');
      return;
    }
    setSaving(true);
    try {
      // 1. Upload foto lewat endpoint dokumentasi penugasan
      const formData = new FormData();
      formData.append('file', file);
      formData.append('jenis_dokumentasi', 'monitoring_tanaman');
      formData.append('keterangan', `Monitoring tanaman #${item.id}`);
      ...
      const fotoUrl = filePath ? `${STORAGE_BASE_URL}/storage/${filePath}` : null;

      // 2. Simpan data monitoring tanaman
      await axios.put(
      ...
      console.error('Gagal menyimpan data monitoring:', err);
      toast.error(err?.response?.data?.message || 'Gagal menyimpan data monitoring.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl relative animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar flex flex-col gap-5">
          ...
          <div className="border border-emerald-500 bg-[#F8FBF9] rounded-xl p-4 flex flex-col border-dashed">
            <label className="text-xs font-bold text-gray-800 block mb-0.5">Foto Monitoring <span className="text-red-500">*</span></label>
            <p className="text-[10px] text-gray-500 font-medium mb-3">Foto terbaru saat monitoring (maks. 5MB)</p>
            {preview ? (
              ...
              <div className="flex flex-col items-center justify-center text-center py-6">
                ...
                <span className="text-xs font-bold text-emerald-700">Belum ada foto dipilih</span>
              </div>
            )}
