...
const DetailBibit: React.FC = () => {
  ...
  const [bibitData, setBibitData] = useState<BibitResponseData | null>(null);
  const [specsData, setSpecsData] = useState<SeedSpecResponseData[]>([]);
  ...
  const fetchDetailData = async () => {
    ...
    try {
      ...
      const relatedSpecs = allSpecsRes.payload.filter(
        (spec) => spec.seed_id === bibit.id
      );
      setSpecsData(relatedSpecs);
  ...
  const formatTinggi = (min: number, max: number) => {
    if (max === 0 || min > 100) return `> 100 cm`;
    return `${min} - ${max} cm`;
  };
  ...
  if (!bibitData) {
    return (
      <div className="w-full py-20 text-center text-gray-500 font-bold">
  ...
  return (
    <div className="w-full mx-auto pb-12">
      <div className="flex items-center justify-between mb-8">
        ...
        <button 
          onClick={() => navigate(`/admin/staff/donasi/bibit/edit/${id}`)} 
          className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm font-bold rounded-xl hover:bg-gray-50 transition-colors shadow-sm cursor-pointer"
        >
          ...
          <div>
            <div className="flex items-center gap-3 mb-3">
              <span className={`px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full ${
                bibitData.kategori.toLowerCase().includes("kehutanan")
                    ...
                    specsData.map((item) => (
                      <tr key={item.id} className="hover:bg-emerald-50/30 transition-colors">
                        <td className="px-6 py-4 text-sm font-bold text-gray-700">
                          Tinggi Batang {formatTinggi(item.min_height, item.max_height)}
                        </td>
