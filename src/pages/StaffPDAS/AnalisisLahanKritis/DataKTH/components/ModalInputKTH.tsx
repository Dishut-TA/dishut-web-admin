...
import { createKthAPI, importKthExcelAPI } from '@/services/kth.service'; 
...
const ModalInputKTH: React.FC<ModalInputKTHProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const loadingToast = toast.loading('Memproses data KTH...');
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
          ...
          <form onSubmit={handleSubmit} className="relative">
            ...
            <div className={`grid transition-all duration-500 ease-in-out ${inputMode === 'manual' ? 'grid-rows-[1fr] opacity-100 translate-x-0 scale-100' : 'grid-rows-[0fr] opacity-0 translate-x-4 scale-95 pointer-events-none absolute inset-0'}`}>
              <div className="overflow-hidden">
                <div className="space-y-4 p-1">
                  
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5 ml-1">Cabang Dinas Kehutanan</label>
                    <select name="cdk" value={formData.cdk} onChange={handleInputChange} required={inputMode === 'manual'} tabIndex={inputMode === 'manual' ? 0 : -1} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50/50 hover:bg-gray-50 outline-none focus:ring-2 focus:ring-[#185325]/20 focus:border-[#185325] transition-all cursor-pointer">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input name="kabupaten_kota" value={formData.kabupaten_kota} onChange={handleInputChange} placeholder="Kab. Bandung" required={inputMode === 'manual'} tabIndex={inputMode === 'manual' ? 0 : -1} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50/50 hover:bg-gray-50 outline-none focus:ring-2 focus:ring-[#185325]/20 focus:border-[#185325] transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1.5 ml-1">Kecamatan</label>
                      <input name="kecamatan" value={formData.kecamatan} onChange={handleInputChange} placeholder="Kecamatan" required={inputMode === 'manual'} tabIndex={inputMode === 'manual' ? 0 : -1} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50/50 hover:bg-gray-50 outline-none focus:ring-2 focus:ring-[#185325]/20 focus:border-[#185325] transition-all" />
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5 ml-1">Desa/Kelurahan</label>
                    <input name="desa_kelurahan" value={formData.desa_kelurahan} onChange={handleInputChange} placeholder="Desa/Kelurahan" required={inputMode === 'manual'} tabIndex={inputMode === 'manual' ? 0 : -1} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50/50 hover:bg-gray-50 outline-none focus:ring-2 focus:ring-[#185325]/20 focus:border-[#185325] transition-all" />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5 ml-1">Nama Kelompok & Ketua</label>
                    <div className="flex gap-4">
                      <input name="nama" value={formData.nama} onChange={handleInputChange} tabIndex={inputMode === 'manual' ? 0 : -1} type="text" placeholder="Nama KTH" required={inputMode === 'manual'} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50/50 hover:bg-gray-50 outline-none focus:ring-2 focus:ring-[#185325]/20 focus:border-[#185325] transition-all" />
                      <input name="ketua" value={formData.ketua} onChange={handleInputChange} tabIndex={inputMode === 'manual' ? 0 : -1} type="text" placeholder="Nama Ketua" required={inputMode === 'manual'} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50/50 hover:bg-gray-50 outline-none focus:ring-2 focus:ring-[#185325]/20 focus:border-[#185325] transition-all" />
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1.5 ml-1">Jenis Usaha</label>
                    <select name="jenis_usaha" value={formData.jenis_usaha} onChange={handleInputChange} required={inputMode === 'manual'} tabIndex={inputMode === 'manual' ? 0 : -1} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50/50 hover:bg-gray-50 outline-none focus:ring-2 focus:ring-[#185325]/20 focus:border-[#185325] transition-all cursor-pointer">
            ...
            <div className={`pt-6 border-t border-gray-100 ${inputMode === 'manual' ? 'mt-4' : 'mt-12'}`}>
              <button 
                type="submit" 
              ...
              >
                ...
                {isSubmitting ? 'Memproses...' : `Submit Data ${inputMode === 'excel' ? 'Excel' : 'Manual'}`}
              </button>
            </div>
